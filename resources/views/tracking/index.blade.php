                        ...
                        @foreach(['in_progress' => 'กำลังดำเนินการ (In Progress)', 'completed' => 'เสร็จสิ้น (Completed)', 'rejected' => 'ยกเลิก (Rejected)'] as $groupKey => $groupTitle)
                            ...
                            @else
                                ...
                                @foreach($filteredRequests as $index => $req)
                                    @php
                                        // Progress is calculated dynamically from actual checked steps
                                        // (computed below after all step booleans are set)
                                        
                                        $step1 = true;
                                        $step2 = in_array($req->status, ['submitted', 'approved_manager', 'approved_ict', 'approved_cao', 'pr_created', 'pr_approved_ict', 'pr_approved_cao', 'po_created', 'delivered', 'completed']);
                                        $step3 = in_array($req->status, ['approved_manager', 'approved_ict', 'approved_cao', 'pr_created', 'pr_approved_ict', 'pr_approved_cao', 'po_created', 'delivered', 'completed']);
                                        $step4 = in_array($req->status, ['approved_cao', 'pr_created', 'pr_approved_ict', 'pr_approved_cao', 'po_created', 'delivered', 'completed']);
                                        ...
                                        // Dynamic progress: count checked steps out of 9 total
                                        // Steps: step1, step2, step3, step4, step5, step_pr_ict, step_pr_cao, step6, step8
                                        if ($req->status === 'rejected') {
                                            $progress = 0.00;
                                        } else {
                                            $checkedCount = ($step1 ? 1 : 0)
                                                + ($step2 ? 1 : 0)
                                                + ($step3 ? 1 : 0)
                                                + ($step4 ? 1 : 0)
                                                + ($step5 ? 1 : 0)
                                                ...
                                                + ($step6 ? 1 : 0)
                                                + ($step8 ? 1 : 0);
                                            $progress = round($checkedCount / 9 * 100, 1);
                                        }
                                        ...
                                        if ($req->status === 'draft') {
                                            ...
                                            $ownerText = 'รออนุมัติแผนก';
                                            $ownerColor = 'text-amber-700 bg-amber-50 border-amber-200';
                                        } elseif (in_array($req->status, ['approved_manager', 'pr_created'])) {
                                            $currentOwner = 'ict';
                                            $ownerText = 'รอ Manager ICT';
                                            $ownerColor = 'text-indigo-800 bg-indigo-50 border-indigo-200';
                                        } elseif (in_array($req->status, ['approved_ict', 'pr_approved_ict'])) {
                                            $currentOwner = 'cao';
                                            $ownerText = 'รอ CAO อนุมัติ';
                                            ...
                                            $ownerColor = 'text-blue-700 bg-blue-50 border-blue-300';
                                        } elseif ($req->status === 'delivered') {
                                            $currentOwner = 'accounting';
                                            $ownerText = 'รอฝ่ายบัญชี';
                                            $ownerColor = 'text-teal-700 bg-teal-50 border-teal-200';
                                        }
                                    @endphp
                                    <tr class="data-row hover:bg-slate-50/80 transition-colors" data-current-owner="{{ $currentOwner }}" data-group="{{ $groupKey }}">
                                        ...
                                        <td class="border border-slate-200 py-2 px-3 font-semibold text-slate-900 font-sans bg-white">
                                            ...
                                            <div class="flex items-center gap-1.5 flex-wrap mt-0.5">
                                                ...
                                                <span class="inline-block px-1.5 py-0.5 rounded text-[9px] font-bold border {{ $ownerColor }} whitespace-nowrap">{{ $req->status_text }}</span>
                                            </div>
                                        </td>
